...
% profile 1
id{1}=1;
subs{1}=[1,1,1];      % start from index '1'
...
% profile 3, y-slice
id{3}=3;
subs{3}=[1,1,1];      % start from index '1'
...
savegif = 1;
gif_fnm = ['snapshot_anni_surf_multi_' varnm '.gif'];
...
flag_print  = 1;
flag_light  = 1;
flag_quarter_empty  = 1;

scl_caxis=[-1.0 1.0]*1e-2;
scl_daspect =[1 1 1];
clrmp       = 'jetwr';
taut=0.5;
% ---------------------------------------------------------------------- %
...
for nlayer=ns:nt:ne
    ...
    for i=1:length(subs)
        ...
        %-- set NaN for this example run
        if flag_quarter_empty == 1
            if i == 1  % for top layer
            %if nlayer > 300
              v{i}(1:80,1:49,1) = NaN; %-- acoording to .src
            %end
            end
        end
    ...
    if savegif
      ...
      if nlayer==ns
        imwrite(imind,map,gif_fnm,'gif','LoopCount',Inf,'DelayTime',0.5);
      else
        imwrite(imind,map,gif_fnm,'gif','WriteMode','append','DelayTime',0.5);
      end
    end
