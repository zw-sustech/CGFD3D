...
for n = 1 : num_of_intfce
  ...
  point_Qp{n} = zeros(ny, nx, num_of_point_per_lay(n));
  point_Qs{n} = zeros(ny, nx, num_of_point_per_lay(n));
end
...
for i = 1 : nx
  ...
  for ipt = 1 : num_of_point_per_lay(ilay)
    ...
    point_Qp  {ilay}(j,i,ipt) =  Qp(ilay);
    point_Qs  {ilay}(j,i,ipt) =  Qs(ilay);
...
%     shading faceted;
    ...
    colorbar;
    title('Qp at top of each layers');
end
...
%-- permute if order is not [x,y,layer]
for n = 1 : num_of_intfce
  md.val{n}    = permute(point_Qp{n},[2,1,3]);
end
...
fnm_ou = 'topolay_vis_iso_Qp.md3grd'
...
%-- permute if order is not [x,y,layer]
for n = 1 : num_of_intfce
  md.val{n}    = permute(point_Qs{n},[2,1,3]);
end
...
fnm_ou = 'topolay_vis_iso_Qs.md3grd'
