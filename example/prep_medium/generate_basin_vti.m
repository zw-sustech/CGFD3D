...
fprintf(fid, '%d %d %f %f %f %f\n', nx, ny, x0, y0, dx, dy);
  for ilay = 1 : num_of_layer+1
      for j = 1 : ny
          for i = 1 : nx
            	fprintf(fid, '%f %f %f %f\n', elev_int(j,i,ilay), ...
                    epsilon(ilay), epsilon_grad(ilay), epsilon_pow(ilay));
          end
      end
  end
fclose(fid);

fnm_ou = 'basin_delta.md3lay'
fid = fopen(fnm_ou,'w');
fprintf(fid, '%d\n', num_of_layer + 1);
fprintf(fid, '%d %d %f %f %f %f\n', nx, ny, x0, y0, dx, dy);
  for ilay = 1 : num_of_layer+1
      for j = 1 : ny
          for i = 1 : nx
            	fprintf(fid, '%f %f %f %f\n', elev_int(j,i,ilay), ...
                    delta(ilay), delta_grad(ilay), delta_pow(ilay));
          end
      end
  end
fclose(fid);

fnm_ou = 'basin_gamma.md3lay'
fid = fopen(fnm_ou,'w');
fprintf(fid, '%d\n', num_of_layer + 1);
fprintf(fid, '%d %d %f %f %f %f\n', nx, ny, x0, y0, dx, dy);
  for ilay = 1 : num_of_layer+1
      for j = 1 : ny
          for i = 1 : nx
            	fprintf(fid, '%f %f %f %f\n', elev_int(j,i,ilay), ...
                    gamma(ilay), gamma_grad(ilay), gamma_pow(ilay));
          end
      end
  end
fclose(fid);
