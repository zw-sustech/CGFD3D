/*******************************************************************************
 * Staggered Grid Finite Difference Acoustic Wave Propagation Simulation 
 *
...
//-------------------------------------------------------------------------------
  ...
  int num_level = 1;
  wav_ac_init(gdinfo, wav, num_level);
...
//-------------------------------------------------------------------------------
  ...
  if (par->bdry_has_cfspml == 1)
  {
    ...
    bdry_pml_set(gdinfo, gdcart, wav, bdry,
                 mympi->neighid,
                 par->cfspml_is_sides,
                 par->abs_num_of_layers,
...
//-------------------------------------------------------------------------------
  ...
  blk_stg_ac_mesg_init(mympi, gdinfo->ni, gdinfo->nj, gdinfo->nk,
                  fd->fdx_nghosts, fd->fdy_nghosts);
