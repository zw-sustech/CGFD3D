function [tfr,t,f]=tfrunter(X,time,form,fmin,fmax,N,trace); 
%TFRUNTER Unterberger time-frequency distribution.
%	[TFR,T,F]=TFRUNTER(X,T,FORM,FMIN,FMAX,N,TRACE) generates the
%	auto- or cross-Unterberger distribution (active or passive form).
%
%	X : signal (in time) to be analyzed. If X=[X1 X2], TFRUNTER 
%	   computes the cross-Unterberger distribution (Nx=length(X)).
%	T : time instant(s) on which the TFR is evaluated  
%	    					(default : 1:Nx).
%	FORM : 'A' for active, 'P' for passive Unterberger distribution.
%						(default : 'A'). 
...
%	   frequency. When called without output arguments, TFRUNTER
%	   runs TFRQVIEW.
...
elseif (form~='A' & form~='P'),
 error('Unknown value for FORM'); 
end; 

Mt = length(X); 

if trace, disp('Unterberger distribution'); end;
